<template>
  ...
  <div class="space-y-6">
    <div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
      <div>
        <h1 class="text-2xl font-semibold text-slate-900">Attendants</h1>
        <p class="mt-1 text-sm text-slate-500">View and manage all registered attendants</p>
      </div>
    </div>
    ...
    <Card>
      <div class="space-y-4">
        ...
        <EmptyState
          ...
          description="No attendants match your search criteria."
        />
...
const filters = ref({
  ...
  membership: props.filters?.membership || '',
});

const applyFilters = () => {
  router.get('/admin/attendants', filters.value, {
    preserveState: true,
    preserveScroll: true,
  });
};
...
const exportUrl = (format) => {
  const params = new URLSearchParams({ type: 'all', format });
